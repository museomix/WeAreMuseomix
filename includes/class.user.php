	...
	*/
	...
	function printData($level=0,$before=NULL,$after=NULL) {
		...
		if ( $level == 0 ) { // global data / my account / private
			...
			$string .= "<p>"._('Permalien')." : <a href='{$this->getPermalink()}'>{$this->getPermalink()}</a></p>" ;
		}
		...
		if ( $level == 2 ) { // simple bausson template
			$string .= "<div style='width:100%;background-color:#808080;padding-top:50px;padding-bottom:50px;'>" ;
			$string .= "<p align='center'>" ;
			$string .= $this->getImage(640,"bausson") ;
			$string .= "</p>" ;
			$string .= "</div>" ;
		}
		...
		if ( $level == 4 ) { // page user
			...
			$string .= _('permalink')." <a href='{$this->getPermalink()}'>{$this->getPermalink()}</a></p>" ;		
		}
